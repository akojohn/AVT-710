...
class Controller implements StatisticForm.OnDialogListener {
    ...
    private void initializeMainForm() {
        ...
        mainForm.getConnectButton().addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                System.out.println("Создание потока онлайн соединения");
                clientSocket = new ClientSocket(mainForm);
            }
        });

        mainForm.getDisconnectButton().addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                System.out.println("Отправка команды на отключение");
                clientSocket.disconnect();
                mainForm.getDlm().clear();
                mainForm.setTitle("Aquarium");

                mainForm.getConnectButton().setEnabled(true);
                mainForm.getDisconnectButton().setEnabled(false);
                mainForm.getImportButton().setEnabled(false);
            }
        });

        mainForm.getImportButton().addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if(mainForm.getDlm().size() > 0){
                    System.out.println("Отправка запроса на импорт");
                    UUID uuid = UUID.fromString(mainForm.getDlm().get(mainForm.getIpsList().getSelectedIndex()));
                    clientSocket.importFrom(uuid);
                }
                else{
                    System.out.println("Не выбран или отсутствует клиент для запроса на импорт");
                }
            }
        });
    }
